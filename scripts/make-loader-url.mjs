...
const bookmarklet = `javascript:(async()=>{const i='nico-neo-loader';if(document.getElementById(i))return;let v='${branch}';try{const r=await fetch('${api}',{cache:'no-store',credentials:'omit',headers:{Accept:'application/vnd.github+json'},referrerPolicy:'no-referrer'});if(r.ok){const j=await r.json();if(typeof j.sha==='string'&&/^[0-9a-f]{40}$/.test(j.sha))v=j.sha}}catch{}const s=document.createElement('script');s.id=i;s.charset='UTF-8';s.src='${cdn}'+v+'/dist/bookmarklet.js'+(v==='${branch}'?'?v='+Date.now():'');s.onload=()=>s.remove();s.onerror=()=>{s.remove();alert('NicoNEOの読み込みに失敗しました。')};(document.head||document.documentElement).appendChild(s)})()`;
